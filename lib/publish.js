...
function buildPluginYaml(entry) {
  ...
  if (entry.image_digest) lines.push(`image_digest: "${entry.image_digest}"`);
  lines.push(`license: ${entry.license}`);
  if (entry.manifest_sha256) lines.push(`manifest_sha256: ${entry.manifest_sha256}`);
...
async function publish(args = {}) {
  ...
  // ── Step 7: Compute manifest SHA-256 ───────────────────────────
  ...
  try {
    ...
    if (ci) {
      console.error(JSON.stringify({ error: "manifest_fetch_failed", message: err.message }));
      process.exit(1);
    }
    console.error(`  \x1b[33m\u26A0\x1b[0m Could not fetch manifest to compute hash: ${err.message}`);
    console.error("  The manifest_sha256 field will be omitted.\n");
  }

  // ── Step 8: Get Docker image digest ────────────────────────────

  log("  Step 6: Resolving Docker image digest...\n");
  let imageDigest = null;
  if (manifest.image) {
    imageDigest = getImageDigest(manifest.image);
    if (imageDigest) {
      log(`  \x1b[32m\u2714\x1b[0m image_digest: ${imageDigest.slice(0, 23)}...\n`);
    } else {
      if (ci) {
        console.error(JSON.stringify({
          warning: "image_digest_unavailable",
          message: `Could not resolve digest for ${manifest.image}`,
        }));
      } else {
        console.error(`  \x1b[33m\u26A0\x1b[0m Could not resolve image digest for ${manifest.image}`);
        console.error("  Make sure the image is pushed and accessible.\n");
      }
    }
  }
